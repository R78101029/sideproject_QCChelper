...
同仁在執行品管圈時常在步驟間來回修改，或卡在某個步驟不知如何推進。AI Agent 可綜合分析整個專案狀態，主動提供指引。

**觸發方式：**
- 同仁可在任何步驟點擊「分析專案」按鈕，請 AI 檢視整個專案
- 系統偵測到同仁在同一步驟停留過久（如超過預設天數）時，主動提示
- 進入專案總覽頁面時，自動顯示健康度摘要
...
| 分析面向 | 說明 |
|---------|------|
| 進度對比 | 對比甘特圖預定進度與實際進度，標示落後的步驟 |
| 步驟完成度 | 檢查各步驟必填欄位是否已填齊，列出缺漏項目 |
| 數據一致性 | 檢查步驟間數據是否一致（如：步驟五的現狀值是否與步驟四數據吻合） |
| 卡關診斷 | 針對目前停滯的步驟，分析可能原因並建議下一步行動 |
| 品質建議 | 依已填內容給予品管手法上的改善建議（如：魚骨圖要因是否足夠、真因驗證是否充分） |

**建議輸出範例：**
- 「步驟四的查檢數據已上傳，但步驟五的目標尚未設定。建議先完成目標設定，以利後續解析。」
...
**連動規則：**
- 上游步驟修改數據時，下游步驟若已有資料，系統顯示**黃色警示**提示同仁確認
- 下游步驟的「自動帶入」欄位會即時更新，但不會覆蓋同仁手動修改過的值
- 同仁可點擊「重新同步」按鈕，強制以上游最新數據覆蓋

---

### 3.9 步驟完成度檢核

每個步驟設有**必填項目**與**選填項目**的區分，確保同仁不會遺漏關鍵資料。

**檢核機制：**

| 機制 | 說明 |
|------|------|
| 必填標記 | 各步驟的必填欄位以紅色星號標示 |
| 即時檢核 | 填寫過程中即時顯示已完成 / 未完成的必填項目數量 |
| 步驟鎖定 | 必填項目未填齊前，該步驟無法標記為「已完成」（但可暫存為「進行中」） |
| 完成度百分比 | 在專案總覽顯示各步驟的填寫完成度（含必填＋選填） |
| 匯出前檢查 | 匯出報告前，系統列出所有步驟的缺漏項目，提醒同仁補齊 |
...
品管圈管理員專屬的管理介面，用於掌握全院品管圈活動狀態。

| 功能 | 說明 |
|------|------|
| 全院進度總覽 | 所有圈隊的進度一覽表：目前步驟、預定完成日、是否落後 |
| 卡關預警 | 自動標示停滯超過 N 天未有進展的圈隊（天數可設定） |
| 即將到期提醒 | 列出活動期間即將結束但尚未完成的圈隊 |
| 年度統計 | 本年度圈隊數量、完成率、平均活動週數、各科參與統計 |
| 圈隊評比 | 各圈的改善幅度、目標達成率排名（用於年度評比） |
| 批次匯出 | 一次匯出多個圈隊的報告（用於呈報或評審） |

---

### 3.12 輔導紀錄與月會輸出

每月外聘老師來院指導時，圈隊代表與管理員需要快速產出目前進度的摘要資料。

#### 圈隊代表的輔導準備輸出

| 功能 | 說明 |
|------|------|
| 進度摘要報告 | 一鍵產出目前專案的簡要進度報告（PDF），包含：已完成步驟摘要、目前步驟進展、遇到的問題 |
| 本月異動摘要 | 自動彙整自上次輔導以來的所有變更（新增/修改的步驟內容、上傳的資料） |
| 待討論事項 | 同仁可標記需要在輔導會議中討論的問題，系統自動收集成清單 |
| 圖表快速列印 | 將目前已產生的所有圖表（柏拉圖、魚骨圖等）一次列印或匯出為 PDF |

#### 管理員的月會報告輸出

| 功能 | 說明 |
|------|------|
| 全院進度總表 | 匯出所有圈隊的進度一覽表（Excel/PDF），含各圈目前步驟、落後狀態 |
| 輔導紀錄表 | 提供輔導紀錄表範本（Word/PDF），老師可在表上填寫建議 |
| 上次建議追蹤 | 列出上次輔導老師的建議事項，與目前圈隊的改善狀況對照 |
| 輔導歷程 | 各圈歷次輔導紀錄的時間軸檢視 |
...
| 功能 | 說明 |
|------|------|
| 記錄輔導建議 | 管理員或圈隊代表可在輔導後記錄老師的建議與指導重點 |
| 綁定步驟 | 輔導建議可關聯到特定步驟，方便後續追蹤 |
| 追蹤狀態 | 每條建議可標記為「待處理/處理中/已完成」 |
